...
export default function RepriseAlimentaireApresJeune() {
  ...
  // Déterminer le jour courant de la reprise (si la date de début est passée)
  ...
  // Afficher uniquement J+1 et J+2 si la reprise n'a pas commencé
  let joursAAfficher = [];
  if (jours.length > 0) {
    if (jourReprise < 1) {
      joursAAfficher = jours.slice(0, 2); // J+1 et J+2
    } else {
      joursAAfficher = jours.slice(0, jourReprise); // Affiche jusqu'au jour courant
    }
  }
